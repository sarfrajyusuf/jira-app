...
export const CycleStartModal: React.FC<CycleModalProps> = (props) => {
...
const handleFormSubmit = async (formData: Partial<ICycle>, dirtyFields: any) => {
    ...
    if (data && payload.start_date && payload.end_date) {
      if (data?.start_date && data?.end_date) {
        await handleUpdateCycle(data.id, payload, dirtyFields);
      } else {
        setToastAlert({
          type: "error",
          title: "Error!",
