...
export const CycleStartForm: React.FC<Props> = (props) => {
  const { handleFormSubmit, handleClose, status, projectId, setActiveProject, data } = props;
  // form data
  const dateNow= renderFormattedPayloadDate(new Date());
  const {
    ...
    <form onSubmit={handleSubmit((formData)=>handleFormSubmit(formData,dirtyFields))}>
      <div className="space-y-5">
        <div className="flex items-center gap-x-3">
          ...
          <h3 className="text-xl font-medium leading-6 text-custom-text-200"> Sprint</h3>
        </div>
        <div className="space-y-3">
          <div className="mt-2 space-y-3">
            <h1>Do you want to start this sprint ?</h1>
            {/* <div>
              <Controller
                name="name"
                control={control}
